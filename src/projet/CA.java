...
public class CA {
	...
	/**
	 * Cette méthode lit le contenu d'un fichier texte pour extraire des informations
	 * sur les villes, les routes et les bornes de recharge, et les stocke dans des listes.
	...
	/**
	 * Cette méthode écrit les données des villes, routes et bornes de recharge dans un fichier texte.
	 * 
	    ...
	    if (fichier.exists()) {
	        try (FileWriter writer = new FileWriter(cheminFichier)) {
	            ...
	            for (Route route : routes) {
	                writer.write("route(" + route.getVilleSource().getNom() + "," + route.getVilleCible().getNom() + ").\n");
	            }
	...
	/**
	 * Cette méthode ajoute chaque ville de la liste des villes à la liste des bornes de recharge.
	 */
	 ...
	 * @throws InvalidInputException Si la ville existe déjà dans la liste des villes
	 */
	...
	/**
	 * Supprime une ville de la liste des villes et des bornes de recharge.
	 * Supprime également toutes les routes associées à cette ville.
	 * 
	 * @param ville La ville à supprimer
	 * @throws InvalidInputException Si la ville n'existe pas dans la liste des villes
	 */
	...
	/**
	 * Ajoute une route à la liste des routes si les villes sources et cibles existent dans la liste des villes.
	 * 
	 * @param route La route à ajouter
	 * @throws InvalidInputException Si au moins l'une des villes sources ou cibles n'existe pas,
	 * ou si la route existe déjà
	 */
	...
	/**
	 * Supprime une route de la liste des routes si les villes sources et cibles existent dans la liste des villes
	 * et si la route entre ces deux villes existe.
	 * 
	 * @param route La route à supprimer
	 * @throws InvalidInputException Si au moins l'une des villes sources ou cibles n'existe pas, ou si la route n'existe pas
	 */
	...
	/**
	 * Ajoute une borne de recharge à une ville si elle existe dans la liste des villes 
	 * et si elle n'a pas déjà de borne de recharge.
	 * 
	 * @param ville La ville à laquelle ajouter une borne de recharge
	 * @throws InvalidInputException Si la ville n'existe pas ou si elle a déjà une borne de recharge
	 */
	...
	/**
	 * Vérifie si une ville (villeCible) est accessible, c'est à dire si elle a une borne de recharge
	 * ou si elle a une ville a une ville adjacente qui a une borne de recharge à part celle qu'on 
	 * veut supprimer sa borne de recharge (villeSource).
	 * 
	 * @param villeSource La ville source (celle qu'on veut supprimer sa borne de recharge)
	 * @param villeCible La ville cible (celle qu'on veut savoir si elle vérifie le crritère d'accessibilité
	...
	public boolean isAccessible(Ville villeSource, Ville villeCible) {
		...
		if(bornes.contains(villeCible)) {
			res = true;
		/* si elle n'a pas de borne de recharge, on cherche si elle a une ville adjacente
		 * qui a une borne de recharge
		 * à part celle qu'on veut supprimer	
		*/
		} else {
			for(Route route:routes) {
				if((route.getVilleSource().equals(villeCible)) &&
						!(route.getVilleCible().equals(villeSource)) && 
						(bornes.contains(route.getVilleCible()))) {
					res = true;	
				} else if((route.getVilleCible().equals(villeCible)) &&
						!(route.getVilleSource().equals(villeSource)) && 
	...
	/**
	 * Supprime la borne de recharge d'une ville si elle existe et contient une borne de recharge 
	 * et si le critère d'accessibilité est vérifié.
	 * 
	 * @param ville La ville dont la borne de recharge doit être supprimée
	 * @throws InvalidInputException Si la ville n'existe pas ou si elle n'a pas de borne de recharge,
	 *                               ou si la suppression de la borne laisserait une ville adjacente
	 *                               sans borne de recharge (critère d'accessibilité non vérifié)
	 */
		...
		for(Route route: routes) {
			if(route.getVilleSource().equals(ville)) {
				if(!isAccessible(ville,route.getVilleCible())) {
					throw new InvalidInputException("Si on supprime "+ ville.getNom()+
							" alors "+route.getVilleCible().getNom()+ " n'a pas de borne de "
							+ "recharche et n'aura plus de ville adjacente qui en "
							+ "a une. \n");
				}
			}else if(route.getVilleCible().equals(ville)) {
				if(!isAccessible(ville,route.getVilleSource())) {
					throw new InvalidInputException("Si on supprime "+ ville.getNom()+
							" alors "+route.getVilleSource().getNom()+ " n'a pas de borne de "
							+ "recharche et n'aura plus de ville adjacente qui en "
							+ "a une. \n");
				}
			}
	...
	/**
	 * Vérifie si chaque ville a une borne de recharge ou si au moins une ville adjacente a une borne de recharge.
	 * 
	...
	/**
	 * Initialise la liste des bornes de recharge avec toutes les villes qui ne sont pas encore des bornes.
	 * Elle ajoute les villes manquantes à la liste des bornes de recharge.
	 */
	 ...
	 * 
	 * @param ville La ville pour laquelle on veut connaître le nombre de routes connectées
	...
	/**
	 * Récupère la ville cible associée à une ville source donnée, à travers les routes existantes.
	 * 
	 * @param ville La ville source pour laquelle on cherche la ville cible
	 * @return La ville cible associée à la ville source donnée, ou null si elle n'est pas trouvée
	 */
	...
	/**
	 * Récupère la liste des villes adjacentes à une ville donnée à travers les routes.
	 * 
	...
	/**
	 * Tente d'améliorer l'optimisation en supprimant une à une les bornes de recharge.
	 * Si une exception est levée lors de la suppression d'une borne, affiche un message indiquant
	 * qu'on ne peut pas améliorer davantage.
	 */
	...
	/**
	 * Identifie les villes qui nécessitent une borne de recharge obligatoire en raison de leur seule route sortante.
	 * 
	...
	/**
	 * Optimise l'emplacement des bornes de recharge en se basant sur plusieurs critères.
	 * Cet algorithme se base sur deux approches: 
	 * 		-la première s'agit d'identifier les villes qui doivent absolument avoir une 
	 * 		une borne de recharge comme elles sont liées à une ville qui a une seule route.
	 * 		-la deuxième est de parcourir toutes les villes en commençant par la ville qui a
	 * 		le plus de nombres de villes adjacentes et on lui met une borne et on retire les 
	 * 		bornes de recharges, si elles existent, de toutes ses villes adjacentes 
	 * 		si la conditino d'accessibilité reste maintenue.
	 *
	 */
	
	public void optimiserBornes() {
/*on utilise bornesObligatoires pour indiquer les villes qui doivent obligatoirement 
 * avoir des bornes comme elles sont liées à une ville qui a une seule route. 
*/
					
/*villesTraitees est utilisée pour supprimer la ville de listBornes lorsqu'on l'aurait
 * déjà traitée ( pour ne pas prendre toujours la meme ville qui a le plus grand nombre
 * de villes adjacentes).
 */
		...
		do {
			...
			for(Ville ville: listVilles) {
				if((getNombreRoutes(ville) > maxVoisins) &&
						!(villesTraitees.contains(ville))) {
			...
			for (Ville villeCible: villesAdj) {
				if(villeCible != null && bornes.contains(villeCible)
					 && !bornesObligatoires.contains(villeCible)) {
				
/* on rajoute cette condition pour etre sur que si une ville avec un plus grand nombre
 * de villes adjacentes, on ne lui retire pas sa borne pour une ville qui a moins de 
 * villes adjacentes.
 */
